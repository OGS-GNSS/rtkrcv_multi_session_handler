...
class Rover(Ricevitore):
    ...
    def process_with_rtkrcv(self, master: Master, rtklib_path: Path, 
        ...
        Args:
            master: Istanza Master con coordinate già impostate
            rtklib_path: Path all'eseguibile rtkrcv
            timeout: Timeout massimo per il fix (secondi)
            
        Returns:
            True se il fix è stato ottenuto e coordinate salvate
        """
        ...
        solution_file = Path(f"solution_{self.serial_number}.pos")
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if process.poll() is not None:
                    print("RTKRCV terminato inaspettatamente")
                    break
...
# Output stream
outstr1-type=file
outstr1-path=solution_%Y%m%d%h%M%S.pos
outstr1-format=llh
...
"""
    ...
    def _check_fix_status(self, line: str) -> bool:
        """Verifica se la linea indica un fix ottenuto"""
        # [Inferenza] Pattern tipici di RTKRCV per fix, da verificare con output reale
        fix_patterns = ['Q=1', 'FIX', 'fixed']
        return any(pattern in line for pattern in fix_patterns)
    
    def _extract_coordinates(self, line: str) -> Optional[Dict[str, float]]:
        """Estrae coordinate da output RTKRCV"""
        # [Inferenza] Pattern di esempio, da adattare al formato reale di output
        try:
            # Cerca pattern tipo: lat lon alt Q
